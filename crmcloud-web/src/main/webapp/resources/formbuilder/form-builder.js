...
function formBuilderHelpersFn(opts, formBuilder) {
    ...
    _helpers.fieldOptionData = function (field) {
        var options = [];
        $('.sortable-options:not(.disabled) li', field).each(function () {
            var $option = $(this),
                    attrs = {
                ...
                if (!$field.hasClass('disabled')) {
                    ...
                    (function () {
                        ...
                        $('[class*="fld-"]', field).each(function () {
                            ...
                            if ($(this).attr('lang')) {
                                $(this).data($(this).attr("lang"), value);
                            }
                            if (!$.isEmptyObject($(this).data())) {
                                value = $(this).data();
                            }
                            ;
                            fieldData[name] = this.type === 'checkbox' ? this.checked : value;
                        });
    ...
    _helpers.save = function () {
        ...
        if (!opts.formIDs)
            opts.formIDs = [];
        opts.formIDs[opts.formID] = true;
        //console.log(opts.formIDs);
        // end hungpd
     ...
     * @return {JSON}
     */
    formBuilder.getData = function getData() {
        var formDataJson = [];
        $(formBuilder.element).find('.frmb.ui-sortable').each(function () {
            var formTitle = $(this).closest('.form-wrap').find('[name="form-header-name"]').val();
            var formId = $(this).closest('.form-wrap').find('[name="form-id"]').val();
            var formColumn = $(this).closest('.form-wrap').find('[name="form-column"]').val();
            var fields = _helpers.prepData(this);

            if(fields.length > 0){
                var formDataItem = {
                    id: parseInt(formId),
                    name: formTitle,
                    col: parseInt(formColumn),
                    fields: fields,
                };
                formDataJson.push(formDataItem);
            }
        });

        return window.JSON.stringify(formDataJson, null, '\t');
    };
    
    formBuilder.getRefId = function(){
        return opts.refId ? opts.refId : 0;
    };
    
    formBuilder.validate = function(){
        _helpers.validateForm();
    };
    ...
    _helpers.updatePreview = function (field) {
        ...
        if (fieldType.match(/(select|checkbox-group|radio-group)/)) {
            ...
            $('.sortable-options li', field).each(function () {
                ...
                if(fieldType === 'select' && option.label === ''){
                    if(option.selected === true && $('.option-value', this)[0].nodeName === 'SELECT'){
                        $.ajax({
                            url: opts.dynamicRestUrl + '/' + option.value,
                            async: false,
                        }).done(function(data){
        ...
        ////20170125
        //multiple select
        $prevHolder.find('select[multiple]').each(function(e){
            $(this).multiselect({
                header: false,
                selectedList: 4,
        ...
        if (fieldType.match(/(select|checkbox-group|radio-group)/)) {
            $('.dropdown-menu.multi-level a', field).on('click', function(e){
        ...
        if(fieldType.match(/(date)/)){
            var dateTimeOpts = $.extend(true, {
                dateFormat: "yy/mm/dd",
                timeFormat: 'HH:mm',
                showButtonPanel: false
            });
            $(field).find('input[type="date"]').each(function(){
                var self = $(this);
                var settingVal = $(field).attr('value');
                self.addClass("form-control");
                self.attr('type', 'text');
                self.css({width: 'calc(100% - 50px)', float: 'left'});
                if(settingVal) self.val(settingVal);
                self.datetimepicker(dateTimeOpts);
                var btn = $('<button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only dropdown-toggle pull-right" style="margin-top: 4px;" type="button" role="button" aria-disabled="false" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">'+
    ...
    _helpers.multiLevelOptionView = function (optionName, attrs) {
        var walk_array = function (optionName, items, level, generate_wrapper) {
            ...
            for (var i = items.length - 1; i >= 0; i--) {
                ...
                options += '<li '+(hasChild ? 'class="dropdown-submenu"' : '')+'><a href="#"><label>';
                if( !hasChild ){
                    options += '<input type="' + (attrs.multiple ? 'checkbox' : 'radio') + '" name="' + optionName + '" value="' + items[i].value + '" ' + selected + '> ';
                }
        ...
        switch (attrs.type) {
            ...
            case 'select':
                ...
                for (i = attrs.values.length - 1; i >= 0; i--) {
                    if( attrs.values[i].hasOwnProperty('items') && attrs.values[i].items.length > 0 ){
                ...
                if( hasMultipleLevel ){
                    ...
                    options = _helpers.multiLevelOptionView(optionName, attrs);
                    //console.log(attrs);
                    preview = 
                            '<button class="ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only dropdown-toggle" type="button" role="button" aria-disabled="false" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> ' +
                            ' <span class="ui-button-icon-left ui-icon ui-c fa fa-th"></span>' + 
                            ' <span class="ui-button-text ui-c">ui-button</span>' + 
                            ' </button>' + 
                            '<span></span>' + 
                            '<ul class="dropdown-menu multi-level" holder="prev-holder-">' + options + '</ul>';
                }else{
                    var options = '',
    ...
    _helpers.confirm = function (message, yesAction) {
        ...
        var btnWrap = utils.markup('div', [yes, no], {className: 'button-wrap'});
    ...
    _helpers.toggleEdit = function (fieldId) {
        ...
        editMode.on('show.bs.modal', function (e) {
            sortables.sortable("option", "disabled", true);
        });

        editMode.on('hidden.bs.modal', function (e) {
...
(function ($) {
    var FormBuilder = function FormBuilder(options, element) {
        ...
        var $sortableFields = $('<ul/>').attr('id', frmbID).addClass('frmb col-2');
        ...
        // hungpd 20161008
        // add button for new section
        opts.formBuilderChilds = [];
        var newSectionButton = $('<button/>', {
            'type': "button",
            'class': 'btn btn-block btn-sm btn-success'
        });
        newSectionButton.text(opts.messages.newSection);
        newSectionButton.appendTo($cbUL);
        newSectionButton.click(function (e) {
            e.preventDefault();
            var formBuilderItem = $(formBuilder.element).formBuilder(opts);
            formBuilderItem.find('.cb-wrap:gt(0)').remove(); //remove controls
            opts.formBuilderChilds.push(formBuilderItem);
        });
        //console.log($(formBuilder.element).find(".cb-wrap"));
        //end hungpd

        // Sortable fields
        $sortableFields.sortable({
            cursor: 'move',
            connectWith: ".frmb.ui-sortable",
            opacity: 0.9,
            revert: 150,
        ...
        $cbUL.sortable({
            ...
            over: function(event, ui){
                var isTextarea = $(ui.item).hasClass('icon-text-area');
                if(isTextarea){
                    $(ui.placeholder).addClass('textarea-field');
                }
            }
        });
        ...
        var $formWrapHeader = $('<div/>', {
            id: frmbID + '-form-wrap-header',
            'class': 'form-wrap-header form-builder' + _helpers.mobileClass()
        });
        $formWrapHeader.html('<input type="text" name="form-header-name" placeholder="' + opts.messages.divName + '"/><input type="hidden" name="form-id" value="0"/><input type="hidden" name="form-column" value="2"/><div class="btn-group pull-right">' +
                '<button type="button" class="btn btn-warning btn-xs dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' +
                '<i class="fa fa-cog" aria-hidden="true"></i>' +
                '</button>' +
                '<ul class="dropdown-menu">' +
                '<li><a href="#" class="change-column" ref="1">' + opts.messages.column1 + '</a></li>' +
                '<li><a href="#" class="change-column" ref="2">' + opts.messages.column2 + '</a></li>' +
                '<li><a href="#" class="change-column" ref="3">' + opts.messages.column3 + '</a></li>' +
                '<li role="separator" class="divider"></li>' +
                '<li><a href="#" class="delete-form"><i class="fa fa-trash-o" aria-hidden="true"></i> ' + opts.messages.delete + '</a></li>' +
                '</ul>' +
                '</div>');
        $formWrapHeader.appendTo($formWrap);

        $formWrapHeader.find(".change-column").click(function (e) {
            e.preventDefault();
            var ref = $(this).attr("ref");
            var f = $(this).closest(".form-wrap");
            var c = f.find(".frmb.ui-sortable");
            c.removeClass("col-1 col-2 col-3").addClass('col-' + ref);
            f.find("[name=form-column]").val(ref);
        });

        $formWrapHeader.find(".delete-form").click(function (e) {
            e.preventDefault();
            var f = $(this).closest(".form-wrap");
            var cfm = confirm("ページを削除確認してください。");
            if (cfm) {
                f.find(".form-wrap-header").remove();
                f.find(".stage-wrap").remove();
                if (f.closest(".ui-tabs-panel").find(".form-wrap-header").length === 0) {
                    var tab = f.closest(".ui-tabs-panel");
                    $(".customize-form").find('a[href="#' + tab.attr("id") + '"] button').attr('no-cfm', 'true').trigger("click");
                }
            }
        });
        ...
        var $stageWrap = $('<div/>', {
            ...
            'class': 'stage-wrap ' // + formBuilder.layout.stage //hungpd
        });

        var cbWrap = $('<div/>', {
            id: frmbID + '-cb-wrap',
            'class': 'cb-wrap ' // + formBuilder.layout.controls //hungpd
        }).append($cbUL[0]);
        ...
        if (opts.showActionButtons) {
            ...
            var viewDataText = opts.dataType === 'xml' ? opts.messages.viewXML : opts.messages.viewJSON,
                    viewData = utils.markup('button', viewDataText, {
                        id: frmbID + '-view-data',
                        type: 'button',
                        className: 'view-data btn btn-sm btn-default'
                    }),
                    clearAll = utils.markup('button', opts.messages.clearAll, {
                        id: frmbID + '-clear-all',
                        type: 'button',
                        className: 'clear-all btn btn-sm btn-default'
                    }),
                    saveAll = utils.markup('button', opts.messages.save, {
                        className: 'btn btn-sm btn-primary ' + opts.prefix + 'save',
                        id: frmbID + '-save',
                        type: 'button'
                    }),
        ...
        $stageWrap.append($sortableFields);
        $stageWrap.before($formWrap);
        $formWrap.append($stageWrap);
        $(element).append($formWrap, cbWrap);
        ...
        if (opts.defaultDivs !== undefined && opts.defaultDivs.length > 0) {
            var div = opts.defaultDivs.shift();
            opts.defaultFields = div.fields;
            //console.log(div);
            $formWrapHeader.find("input[name=form-header-name]").val(div.name);
            $formWrapHeader.find("input[name=form-id]").val(div.id);
            $formWrapHeader.find(".change-column[ref=" + div.col + "]").trigger('click');

            // process data input
            $.each(opts.defaultDivs, function (i, div) {
                newSectionButton.trigger('click');
            });
        } else {
            opts.defaultFields = [];
        }
        ...
        var fieldOptions = function fieldOptions(values) {
            ...
            if (values.type === 'select') {
                ...
                fieldOptions += boolAttribute('multiple', values, labels, 'inline');

                // hungpd 20161101
                // dynamic options
                fieldOptions += '<div class="btn-group btn-group-xs pull-right btn-select-options" role="group">\n\
                        <button type="button" class="btn btn-xs btn-info" ref="static-options" container="option-' + values.name + '">' + opts.messages.static + '</button>\n\
                        <button type="button" class="btn btn-xs btn-default" ref="dynamic-options" container="option-' + values.name + '">' + opts.messages.dynamic + '</button>\n\
                       </div>';
                fieldOptions += '<ol class="sortable-options dynamic-options option-' + values.name + ' disabled" style="display:none">';
                var dynamicFieldValue = false;
                for (i = 0; i < values.values.length; i++) {
                    if(values.values[i].label === ''){
                        dynamicFieldValue = values.values[i];
                        break;
                    }
                }
                fieldOptions += selectDynamicFieldOptions(values.name, dynamicFieldValue);
                fieldOptions += '</ol>';
            }
            fieldOptions += '<ol class="sortable-options static-options option-' + values.name + '">';
            for (i = 0; i < values.values.length; i++) {
        ...
        var advFields = function advFields(values) {
            var advFields = [],
                    ...
                    valueField = function () {
                        var noValField = ['header', 'paragraph', 'file', 'textarea'].concat(optionFields, opts.messages.subtypes.header, opts.messages.subtypes.paragraph);
            ...
            // Khong hien thi toggle trong checkbox
...
//            }
            ...
            // Max length
//            if (values.type === 'text' || values.type === 'textarea' || values.type === 'rich-text'  
//                    || values.type === 'number' || values.type === 'tel' || values.type === 'mail' || values.type === 'password') {
...
//            }
        ...
        var boolAttribute = function boolAttribute(name, values, labels, classes) {
            classes = classes || '';
            ...
            var label = function label(txt) {
                return '<label for="' + name + '-' + lastID + '">' + txt + '</label>';
                    ...
                    checked = values[name] !== undefined && values[name] === true ? 'checked' : '',
            ...
            return '<div class="' + name + '-wrap ' + (classes ? classes : 'form-group') + '">' + inner.join('') + '</div>';
        };
        ...
        var numberAttribute = function numberAttribute(attribute, values) {
            ...
            // Hide Fields
            var hideField = false;
            if (attribute === 'className' || attribute === 'placeholder' || attribute === 'description') {
                hideField = true;
            }
            return '<div class="form-group ' + attribute + '-wrap ' + (hideField ? 'hide' : '') + '"><label for="' + attribute + '-' + lastID + '">' + attrLabel + '</label> ' + numberAttribute + '</div>';
        };
        ...
        var textAttribute = function textAttribute(attribute, values) {
            ...
            // Hide Fields
            var hideField = false;
            if (attribute === 'className' || attribute === 'placeholder' || attribute === 'description' || attribute === 'maxlength') {
                hideField = true;
            }
            ...
            if (!noMakeAttr.some(function (elem) {
                ...
                var extraAttrs = [];
                if ((attribute === 'label' || attribute === 'description') && (typeof attrVal === "object") && !$.isEmptyObject(attrVal)) {
                    $.each(attrVal, function (lang, value) {
                        extraAttrs.push('data-' + lang + '="' + value + '"');
                    });
                    attrVal = attrVal[LANG];
                }

                if (attribute === 'label' && utils.inArray(values.type, textArea) || attribute === 'value' && values.type === 'textarea') {

                    attributefield += '<textarea name="' + attribute + '" placeholder="' + placeholder + '" class="fld-' + attribute + ' form-control" id="' + attribute + '-' + lastID + '" lang="' + LANG + '" ' + extraAttrs.join('') + '>' + attrVal + '</textarea>';
                } else if (attribute === 'label' || attribute === 'description') {
                    attributefield += '<input type="text" value="' + attrVal + '" name="' + attribute + '" placeholder="' + placeholder + '" class="fld-' + attribute + ' form-control" id="' + attribute + '-' + lastID + '" lang="' + LANG + '" ' + extraAttrs.join('') + '>';
                } else {
                ...
                attributefield = '<div class="form-group ' + attribute + '-wrap ' + (hideField ? "hide" : "") + '">' + attributeLabel + ' ' + attributefield + '</div>';
            }
        ...
        var appendNewField = function appendNewField(values) {
            ...
            // Field preview Label
            if ((typeof label === 'object') && !$.isEmptyObject(label)) {
                liContents += '<label class="field-label">' + label[LANG] + '</label>';
            } else {
            ...
            liContents += utils.markup('div', '', {className: 'prev-holder', id: 'prev-holder-'+lastID}).outerHTML;
            liContents += '<div id="' + lastID + '-holder" class="frm-holder modal fade" role="dialog">';

            liContents += '<div class="modal-dialog modal-lg">';
            liContents += '<div class="modal-content">' +
                    '<div class="modal-header" style="position: relative; padding-right: 20px;">' +
                    '<button type="button" class="close" data-dismiss="modal" style="position: absolute; right: 10px; top: 12px; font-size: 17px; color: #000;">&times;</button>' +
                    '<h4 class="modal-title">' + opts.messages.fieldProperties;
            if (opts.language) {
                var langButtons = '<div class="btn-group pull-right" role="group" aria-label="Language">';
                $.each(opts.language, function (k, v) {
                    langButtons += '<button type="button" class="btn btn-lang btn-' + (LANG === v ? 'success' : 'default') + '" ref="' + v + '">' + opts.messages[v] + '</button>';
                });
                langButtons += '</div>';
                liContents += langButtons;
            }
            ...
            var matchWidth = /(^|\s)width-\S+/g.exec(values.className);
            var elmWidth = matchWidth ? matchWidth[0].trim(): '';
            liContents += '<div class="clearfix"></div></div>';
            liContents += '</div>' +
                    '<div class="modal-footer">' +
                    '<div class="btn-group pull-left" role="group" aria-label="Language">' +
                        '<button type="button" class="btn btn-xs btn-width btn-'+(elmWidth === '' ? 'success': 'default')+'" ref="0">'+opts.messages.widthNotSet+'</button>' +
                        '<button type="button" class="btn btn-xs btn-width btn-'+(elmWidth === 'width-25' ? 'success': 'default')+'" ref="25">25%</button>' +
                        '<button type="button" class="btn btn-xs btn-width btn-'+(elmWidth === 'width-50' ? 'success': 'default')+'" ref="50">50%</button>' +
                        '<button type="button" class="btn btn-xs btn-width btn-'+(elmWidth === 'width-70' ? 'success': 'default')+'" ref="75">75%</button>' +
                        '<button type="button" class="btn btn-xs btn-width btn-'+(elmWidth === 'width-100' ? 'success': 'default')+'" ref="100">100%</button>' +
                    '</div>' +
                    '<button type="button" class="btn btn-sm btn-danger" data-dismiss="modal">' + opts.messages.close + '</button>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
            liContents += '</div>';
            ...
            var field = utils.markup('li', liContents, {
                'class': type + '-field form-field ' + elmWidth,
                'type': type,
                id: lastID
            }),
                    $li = $(field);
            ...
            // hungpd 20161030
            $li.find('.btn-lang').click(function (e) {
                e.preventDefault();
                var container = $(this).parent();
                var lang = $(this).attr("ref");
                var modalContent = $(this).closest('.modal-content');
                container.find('.btn.btn-lang').removeClass("btn-success").addClass("btn-default");
                $(this).removeClass("btn-default").addClass('btn-success');
                var labelInput = modalContent.find('[name="label"]');
                labelInput.data(labelInput.attr("lang"), labelInput.val());
                labelInput.attr("lang", lang).val(labelInput.data(lang));

                var descInput = modalContent.find('[name="description"]');
                if (descInput.length) {
                    descInput.data(descInput.attr("lang"), descInput.val());
                    descInput.attr("lang", lang).val(descInput.data(lang));
                }
            });//.first().trigger("click");
            
            $li.find('.btn-width').click(function (e) {
                e.preventDefault();
                var container = $(this).parent();
                var width = $(this).attr("ref");
                var modalContent = $(this).closest('.modal-content');
                container.find('.btn.btn-width').removeClass("btn-success").addClass("btn-default");
                $(this).removeClass("btn-default").addClass('btn-success');
                
                var elmVar = modalContent.find('[name="className"]').val().replace(/(^|\s)width-\S+/g, '');
                if(width) modalContent.find('[name="className"]').val(elmVar + ' width-' + width);

                $(this).closest("li.form-field").attr('class',
                    function(i, c){
                       return c.replace(/(^|\s)width-\S+/g, '');
                    }).addClass(width ? "width-" + width: '');
            });

            if(values.type === 'select'){
                // hungpd 20161101
                $li.find('.btn-select-options .btn').click(function () {
                    var cnt = $(this).attr("container");
                    var ref = $(this).attr('ref');
                    $li.find('.btn-select-options .btn').removeClass('btn-info').addClass('btn-default');
                    $(this).removeClass('btn-default').addClass('btn-info');

                    var allContainer = $li.find('ol.sortable-options.' + cnt);
                    var selectedContainer = $li.find('ol.sortable-options.' + cnt + '.' + ref);

                    allContainer.addClass('disabled').hide();
                    selectedContainer.removeClass('disabled').show();

                    if (ref === 'dynamic-options') {
                        //$li.find('input[name="multiple"]')
                        //        .prop('checked', false)
                        //        .prop('disabled', true)
                        //        .trigger('change');
                        $li.find('.option-actions').hide();
                    } else {
                        //$li.find('input[name="multiple"]')
                        //        .prop('disabled', false);
                        $li.find('.option-actions').show();
                    }
                    
                    $li.find('ol.sortable-options input[type="radio"], ol.sortable-options input[type="checkbox"]').prop('checked', false);
                    selectedContainer.find('input[type="radio"]:first, input[type="checkbox"]:first').prop('checked', true);

                    _helpers.updatePreview($li);
                });
                $.each(values.values, function(index, item){
                    if( item.label === '' && item.selected === true ){
                        $li.find('.btn-select-options .btn[ref="dynamic-options"]').trigger('click');
                        return false;
                    }
                    return true;
                });
        ...
        var selectFieldOptions = function selectFieldOptions(name, optionData, multipleSelect) {
            var optionInputType = {
                ...
                value: 'hidden' // An value field trong dialog selectbox
            },
            ...
            for (var i = optionDataOrder.length - 1; i >= 0; i--) {
                ...
                if (optionData.hasOwnProperty(prop)) {
                    var attrs = {
                        ...
                        'class': 'option-' + prop,
                        value: prop === 'value' && !optionData[prop] ? Date.now() + '-' + i : optionData[prop],
                        name: name
                    };
        ...
        // hungpd 20161101
        // Select field html, since there may be multiple
        var selectDynamicFieldOptions = function selectDynamicFieldOptions(name, optionData) {
            var optionInputType = {
                selected: 'radio'
            },
                    optionDataOrder = ['value', 'label', 'selected'],
            ...
            for (var i = 0; i< optionDataOrder.length; i++) {
                var prop = optionDataOrder[i];
                if (optionData.hasOwnProperty(prop)) {
                    var attrs = {
                        type: optionInputType[prop] || 'text',
                    ...
                    if (prop === 'label') {
                        attrs.class = attrs.class + ' hide';
                    }
                    if (prop === 'selected' || prop === 'label') {
                        optionInputs.push(utils.markup('input', null, attrs));
                    } else {
                        delete attrs.type;
                        delete attrs.placeholder;
                        var selectDataOptions = [];
                        if (opts.dynamicOptions) {
                            $.each(opts.dynamicOptions, function (key, label) {
                                selectDataOptions.push('<option value="' + key + '" ' + (key === optionData.value ? 'selected' : '') + '>' + label + '</option>');
                            });
                        }
                        optionInputs.push(utils.markup('select', selectDataOptions.join(''), attrs));
                    }
                }
            }
        ...
        // update preview to label
        $sortableFields.on('keyup', '[name="label"]', function () {
            var txt = $(this).val();
            var btn = $(this).closest('.modal-content').find('.modal-footer button[data-dismiss]');
            if($.trim(txt).length < 1){
                $(this).attr('placeholder', opts.messages.cannotBeEmpty);
                btn.prop('disabled', 'disabled');
                btn.text(opts.messages.pleaseEnterData);
                btn.removeClass('btn-danger');
                btn.addClass('btn-default');
                alert(opts.messages.cannotBeEmpty);
            }else{
                $('.field-label', $(this).closest('li')).text(txt);
                btn.prop('disabled', null);
                btn.removeClass('btn-default');
                btn.addClass('btn-danger');
                btn.text(opts.messages.close);
            }
        });
        ...
        $sortableFields.on('click', '.add-opt', function (e) {
            ...
            $('.sortable-options:not(.dynamic-options)', $optionWrap).append(selectFieldOptions(name, false, isMultiple));
            _helpers.updateMultipleSelect();
        });
