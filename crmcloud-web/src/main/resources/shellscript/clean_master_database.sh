...
#               sh /mnt/script/clean_master_database.sh 127.0.0.1 3306 root 123456 crmcloud 1 127.0.0.1 3306 root 123456
#
##########################################################################################################################################
...
$MYSQL --user=${MYSQL_MASTER_USER} --password=${MYSQL_MASTER_PASSWORD} --host=${MYSQL_MASTER_HOST} --port=${MYSQL_MASTER_PORT} <<EOFMYSQL
    ...
    INSERT INTO $schema_name.crm_mente_item SELECT * FROM $MYSQL_DATABASE.crm_mente_item WHERE item_level=1 AND company_id=1 AND (item_deleted is null OR item_deleted=0);
    INSERT INTO $schema_name.crm_mente_option_data_value SELECT * FROM $MYSQL_DATABASE.crm_mente_option_data_value WHERE item_id in (SELECT item_id FROM $MYSQL_DATABASE.crm_mente_item WHERE item_level=1 AND company_id=1 AND (item_deleted is null OR item_deleted=0));
    ...
    INSERT INTO $schema_name.crm_project_cust_search_list SELECT * FROM $MYSQL_DATABASE.crm_project_cust_search_list WHERE company_id=1 AND (list_deleted is null OR list_deleted=0);
    
    INSERT INTO $schema_name.crm_zip_code SELECT * FROM $MYSQL_DATABASE.crm_zip_code;
    INSERT INTO $schema_name.crm_config SELECT * FROM $MYSQL_DATABASE.crm_config WHERE config_deleted is null OR config_deleted = 0;
    INSERT INTO $schema_name.crm_database_server SELECT * FROM $MYSQL_DATABASE.crm_database_server;
    INSERT INTO $schema_name.crm_prefecture SELECT * FROM $MYSQL_DATABASE.crm_prefecture WHERE prefecture_is_deleted is null OR prefecture_is_deleted = 0;
